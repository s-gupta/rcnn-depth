...
% Y{1} must be of size 1xN, W{1} must be of size 1x
  ...
  for i = 1:length(param.CMLR),
    trVal = 1;
    tmpModel(i) = trainMCL(sparse(lFeatures{trVal}), Y{trVal}, W{trVal}, param.CMLR(i), param.biasMLR(i), param.mclWeighted); 
    trVal = 2;
  ...
  if(param.weighted == false && ~issparse(x))
    ...
    svmModel = trainSVM(x, y, w, Cs, w1s, biass, param.weighted);
  ...
  mlrModel = trainMCL(sparse(score), YY, w, param.CMLR(mlrParam), param.biasMLR(mlrParam), param.mclWeighted); 
