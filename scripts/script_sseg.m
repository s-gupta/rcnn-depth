...
if strcmp(jobName, 'copy_boxes')
  ...
  s = sprintf('cp %s/detections/*boxes*test*.mat %s/. \n', fullfile(p.detection_dir, 'detector', 'rgb_hha_30000_trainval'), p.ss_det_dir); 
  fprintf(s); 
  % system(s);
  
  s = sprintf('cp %s/detections/*boxes*train*.mat %s/. \n', fullfile(p.detection_dir, 'detector', 'rgb_hha_30000_val'), p.ss_det_dir); 
  fprintf(s); 
  % system(s);
  
  s = sprintf('cp %s/detections/*boxes*val*.mat %s/. \n', fullfile(p.detection_dir, 'detector', 'rgb_hha_30000_train'), p.ss_det_dir); 
  fprintf(s); 
  % system(s);
end
...
if strcmp(jobName, 'deepDetFeatures')
  ...
  dirNameSuffix = 'detection_'; 
  assignTyp = {'bestScore'};
  pThresh = 0.50;
  threshSet = 'val'; boxFieldName = 'boxes';
  K = 1; args = {};
  for i = 1:length(imSet),
    for j = 1:length(assignTyp),
      for k = 1:length(pThresh),
        for l = 1:K,
          imlist = getImageSet(imSet{i});
          args{end+1} = {imSet{i}, ucmThresh, typ, l:K:length(imlist), catName, detDir, assignTyp{j}, pThresh(k), threshSet, dirNameSuffix, nmsThresh, boxFieldName};
        end
      end
    end
  end
  jobParam = struct('numThreads', 1, 'codeDir', pwd(), 'preamble', '', 'matlabpoolN', 4, 'globalVars', {{}}, 'fHandle', @wrapperComputeFeatures4, 'numOutputs', 0);
