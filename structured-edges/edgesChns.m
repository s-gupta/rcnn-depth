function [chnsReg,chnsSim] = edgesChns( I, opts, ng )
...
% BEGIN EDIT BY SG
if(size(I,3) == 4), 
  nTypes=2; Is={I(:,:,1:3),I(:,:,4:end)}; 
elseif(size(I,3) == 5)
  nTypes=3; Is={I(:,:,1:3),I(:,:,4), I(:,:,5)}; 
end
% END EDIT BY SG
%% 
...
%% 
% BEGIN EDIT BY SG
if(exist('ng', 'var')), ng = imResample(ng, 1/shrink); chns = cat(3, chns, ng); end
assert(size(chns,3)==opts.nChns, sprintf('%d, %d - in edgesChns', size(chns,3), opts.nChns));
% END EDIT BY SG
%%
