function [rcnn_model, rcnn_k_fold_model] = ...
    rcnn_region_train(imdb, varargin)
% [rcnn_model, rcnn_k_fold_model] = rcnn_region_train(imdb, varargin)
...
feat_opts_default = struct('fTyp', 'cnn', ...
  'param', struct('pool5dir', 'data/cnnF/cnn_alexnet,rgb,layer_pool5,mode_warpcontext16', 'layer', 5, ...
  'cnnWeightsFile', 'data/caffe/alexnet_layer_weights.mat'));
...
ip.addParamValue('do_latent',       1,      @isscalar);
ip.addParamValue('pos_ov_thresh',   0.5,    @isscalar);
ip.addParamValue('neg_ov_thresh',   0.2,    @isscalar);
...
ip.addParamValue('feat_opts', feat_opts_default, @isstruct); 
...
diary_file = fullfile_ext(conf.cache_dir, ['rcnn_train_' timestamp], 'txt');
diary(diary_file);
fprintf('rcnn_region_train: Logging output in %s\n', diary_file);

fprintf('\n\n~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n');
fprintf('rcnn_region_train: Training options:\n');
disp(opts);
fprintf('~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n\n');

try
  load([conf.cache_dir 'rcnn_model'], 'rcnn_model');
  return
catch
  clear rcnn_model;
end
...
rcnn_model.feat_opts = opts.feat_opts;
% ------------------------------------------------------------------------

% ------------------------------------------------------------------------
% Get the average norm of the features
opts.feat_norm_mean = rcnn_feature_stats(imdb, rcnn_model);
fprintf('rcnn_region_train: average norm = %.3f\n', opts.feat_norm_mean);
...
try
  load(save_file);
  fprintf('rcnn_region_train: Loaded saved positives from ground truth boxes\n');
catch
  [X_pos, keys_pos] = get_positive(imdb, rcnn_model, false);
...
max_hard_epochs = opts.do_latent + 1;
force_update = true;

for hard_epoch = 1:max_hard_epochs,

  if(hard_epoch > 1)
    % Re-estimate the latent positives
  ...
  for i = 1:length(imdb.image_ids)
    fprintf('rcnn_region_train: %s: hard neg epoch: %d/%d image: %d/%d\n', ...
    ...
    for j = imdb.class_ids
      ...
      if (first_time || hit_retrain_limit || is_last_time || force_update) && ...
...
d = get_features(imdb, ind, rcnn_model.feat_opts, false);
...
if first_time
  for cls_id = class_ids
    I = find(d.overlap(:, cls_id) < opts.neg_ov_thresh);
  ...
  for cls_id = class_ids
    ...
    I = find((z > caches{cls_id}.hard_thresh) & ...
             (d.overlap(:, cls_id) < opts.neg_ov_thresh));
...
% ------------------------------------------------------------------------
function [X_pos, keys] = get_positive_features(imdb, rcnn_model, do_latent)
% ------------------------------------------------------------------------
opts = rcnn_model.training_opts;
...
parfor i = 1:length(imdb.image_ids)
  ...
  if do_latent 
    ...
    d(i) = get_features(imdb, i, rcnn_model.feat_opts, true);
  end
end
